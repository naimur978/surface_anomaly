...
**Quick Overview:** If you do not want to go through all the scripts, I have made a brief notebook at `notebook.ipynb`. You can see my work at a glance in the notebook.
...
![GitHub Actions Workflow](source_code/assets/github_actions.png)
...
![MLflow Tracking](source_code/assets/mlflow.png)
...
![Dataset Strategy Comparison](source_code/assets/dataset.png)
...
![ROI Masking Procedure](source_code/assets/roi_procedure.png)
...
![ROI Masking Comparison](source_code/assets/roi.png)
...
![Threshold Comparison](source_code/assets/threshold_comparison.png)
...
I achieved 99.81% image-level AUROC with DINOv2, exceeding the 98% target that I set initially. This demonstrates strong discrimination between normal and anomalous surfaces. It means my model correctly ranks a randomly selected defective image as more anomalous than a normal image 99.81% of the time. The excellent score separation (normal vs. defective) indicates that DINOv2 features capture surface defects very effectively.
...
![ROC Curve](source_code/assets/roc_curve.png)
...
![False Positives](source_code/assets/false_positive.png)
...
I can improve performance with per-defect-type thresholds over the current global threshold (36.0894). Different defect types (scratches, dents, corrosion, contamination) exhibit different anomaly score distributions, so subtle defects may fall below a single global threshold while gross defects exceed it. I would manually label test defects by type, compute separate thresholds per category to achieve 100% recall, and use ensemble voting. However, this requires defect taxonomy (currently unavailable) and more diverse test data than the current 12 defects.

I can also explore ensemble methods and hard negative mining as complementary improvements. Combining multiple backbones (DINOv2 + EfficientNet) increases robustness, while hard negative mining focuses training on borderline false positives to tighten the decision boundary. I believe improved coreset sampling also shows promise by replacing my random 0.15 sampling with methods like k-Center, Importance Sampling, or Stratified Random Sampling. These alternatives would better represent feature space and address current limitations where my random sampling misses pattern variations (e.g., Images 2-3 false positives). While k-Center involves higher training cost (O(n·k) complexity) compared to random sampling (O(n)), it adds no additional inference overhead.

### 11.2 Medium-term

I can explore domain-specific feature extraction via fine-tuning DINOv2 with self-supervised learning (SimCLR, BYOL) to optimize features for subtle surface texture anomalies. Pre-trained ImageNet features are robust but not tailored to this domain, and domain adaptation would learn surface-specific invariances like lighting changes. The main constraint is my limited training data (720 images), which risks overfitting; nevertheless, this approach could tighten the normal boundary and reduce false positives from memory bank gaps.

I also recognize that production monitoring and concept drift detection are essential for deployment. I would implement a monitoring system to log predictions, anomaly scores, and metadata (timestamp, camera condition, batch) to detect performance degradation. Triggers would include FP rate exceeding 2%, detected false negatives, or score distribution shifts (>15% KL divergence). Implementation via ADWIN or statistical tests would enable automatic retraining alerts, accounting for the fact that manufacturing conditions evolve and models degrade over time. Complementary techniques like hierarchical k-NN (multi-scale patch analysis) and anomaly-specific clustering (learning per-defect thresholds) would round out this phase.

### 11.3 Long-term

I believe test-time augmentation (TTA) with diverse models (averaging anomaly scores from DINOv2 and EfficientNet) would provide robustness to unseen defects, though at 2x inference cost. My previous TTA attempts with same-backbone models failed (Section 8), so I would use diverse architectures. I could also apply transformer attention pruning to reduce the memory bank from 27,648 to 5K-10K critical patches, improving efficiency while maintaining accuracy.

I would validate my approach on benchmark datasets (MVTec AD, VisA) with target metrics (<30ms GPU inference) and synthetic defect generation via Flux Schnell or ControlNet. Flux Schnell would generate 200+ synthetic defects (~1 sec/image via text prompts like "scratched metal"), enabling per-defect-type threshold tuning and robustness testing. ControlNet inpainting (2-4 sec/image) would offer photorealistic quality by inpainting defects onto real normal images, supporting contrastive learning. I recommend starting with Flux Schnell for fast, immediate data generation, then graduating to ControlNet for production-grade quality if computational resources permit.
