...
Run: python scripts/benchmark_inference.py [config/config.yaml]

Requires a trained model matching the config's feature_extractor (run
scripts/train.py first). Benchmarks every device available on the machine
(CUDA, MPS, CPU) and prints a comparison table.
"""
...
def measure_inference_time(dataset, model, extractor, device, n_warmup=5, n_samples=10, seed=42):
    """
    ...
    Args:
        ...
        seed: Random seed for sample selection (ensures results are reproducible
            and comparable across devices/runs)

    Returns:
        Array of inference times in seconds
    """
    # Select random samples (seeded so the same images are benchmarked every run)
...
def benchmark_devices(test_dataset, model, extractor, model_name, n_warmup=5, n_samples=10, seed=42):
    """
    ...
    Args:
        ...
        n_warmup: Number of warmup samples
        n_samples: Number of samples to benchmark
        seed: Random seed for sample selection, shared across devices so every
            device benchmarks the exact same images
    ...
    if torch.cuda.is_available():
        ...
        print(f"  [OK] {len(cuda_times)} samples completed")
    ...
    if torch.backends.mps.is_available():
        ...
        print(f"  [OK] {len(mps_times)} samples completed")
    ...
    print(f"  [OK] {len(cpu_times)} samples completed")
...
def main():
    ...
    # Load dataset (suppress prints; context manager restores stdout even on error)
    ...
    roi_status = '[ON] ROI masking' if config['image'].get('apply_roi_mask', False) else '[OFF] No ROI masking'
    print(f"Config: {roi_status}")
