...
def check_devices() -> None:
    ...
    # Devices are checked in priority order (CUDA > MPS > CPU); `selected`
    # is set by whichever tier matches first and falls through to CPU
    # if neither GPU backend is available.
    selected = None
    ...
    if hasattr(torch.backends, 'mps'):
        if torch.backends.mps.is_available():
            print("   [OK] Metal Performance Shaders available")
            if selected is None:
                selected = "MPS"
        else:
            print("   [--] Metal not available or macOS version too old")
    else:
        print("   [--] Not on macOS or PyTorch version doesn't support MPS")

    # Fallback: CPU
    print("\n3. CPU (Fallback)")
    print("-" * 40)
    print("   [OK] CPU always available")
    if selected is None:
        selected = "CPU"
    ...
    try:
        ...
        print("[OK] Tensor operations work correctly")
    ...
    if selected == "CUDA":
        print("""
[OK] CUDA is available - optimal for training
  - Edit config.yaml: device: "cuda"
  - Expect: ~28-30ms per image inference
        """)
    elif selected == "MPS":
        print("""
[OK] Metal Performance Shaders (MacBook GPU) available
