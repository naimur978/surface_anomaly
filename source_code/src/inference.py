...
def infer_ground_truth_label(image_path: str) -> Optional[int]:
    """Infer the ground-truth label (0=normal, 1=defective) from an image's
    path, based on the MVTec-style folder naming convention ('good' vs.
    'broken_*'/'defect_*' subdirectories).

    Returns None if the path doesn't match either convention, meaning no
    ground truth is available for that image (e.g. inference on unlabeled
    production images).
    """
    if 'good' in image_path:
        return 0
    if 'broken' in image_path or 'defect' in image_path:
        return 1
    return None
...
def main() -> None:
    ...
    if args.image:
        ...
        if args.visualize:
            ...
            for idx, result in enumerate(results):
                expected_label = infer_ground_truth_label(result['image_path'])
                expected = {0: 'normal', 1: 'defective'}.get(expected_label)
            ...
            if mismatches:
                ...
                for idx, result, expected, predicted in mismatches:
                    ...
                    try:
                        # Extract heatmap for this mismatch and save with visualization
                ...
                logger.info(f"[OK] Generated {len(mismatches)} heatmap visualizations")
            else:
    ...
    for r in results:
        ...
        expected_label = infer_ground_truth_label(r['image_path'])
        clean['expected'] = {0: 'normal', 1: 'defective'}.get(expected_label, 'unknown')
        clean['expected_label'] = expected_label
    ...
    if results:
        ...
        try:
            ...
            test_labels = [infer_ground_truth_label(result['image_path']) for result in results]
        ...
        if image_files:
            try:
                ...
                if timing_results:
                    ...
                    logger.info("[OK] Inference timing logged to MLflow")
