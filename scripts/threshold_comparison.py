...
def compute_metrics_for_threshold(test_scores, test_labels, threshold):
    ...
    f2 = fbeta_score(test_labels, preds, beta=2, zero_division=0)
...
def main():
    ...
    for key, val in metrics_99.items():
        if key != "threshold":
            print(f"  {key:12s}: {val:.4f}" if isinstance(val, float) else f"  {key:12s}: {val}")

    print("\nMETRICS AT 100% RECALL:")
    metrics_recall = compute_metrics_for_threshold(test_scores, test_labels, recall_100_threshold)
    for key, val in metrics_recall.items():
        if key != "threshold":
            print(f"  {key:12s}: {val:.4f}" if isinstance(val, float) else f"  {key:12s}: {val}")
